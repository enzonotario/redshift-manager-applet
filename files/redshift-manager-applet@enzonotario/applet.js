...
const MONITOR_REAPPLY_COOLDOWN_MS = 2000;
...
class RedshiftApplet extends Applet.TextIconApplet {
    constructor(metadata, orientation, panel_height, instance_id) {
        ...
        this._lastPeriodIsNight = undefined;
        this._monitorCount = global.display.get_n_monitors();
        this._displayChangeTimeoutId = 0;
        this._displayChangeRetryTimeoutId = 0;
        this._applyRedshiftTimeoutId = 0;
        this._lastAppliedTemp = null;
        this._lastAppliedBrightness = null;
        this._lastMonitorReapplyTime = 0;
    ...
    applyPreset(index) {
        ...
        try {
            ...
            if (this.isNightTime()) {
                this.currentTemp = preset.nightTemp;
                this.currentBrightness = preset.nightBrightness;
            } else {
    ...
    adjustActiveTemperature(delta) {
        if (!this.enabled)
            return;
        const isNight = this.isNightTime();
        const key = isNight ? "nightTemp" : "dayTemp";
        const current = this[key] || (isNight ? 3500 : 6500);
        const next = Math.min(10000, Math.max(1000, current + delta));
        if (next === current)
            return;
        this[key] = next;
        this._scheduleApplyRedshift();
        this._queueSaveConfig();
    }
    ...
    adjustActiveBrightness(delta) {
        if (!this.enabled)
            return;
        const isNight = this.isNightTime();
        const key = isNight ? "nightBrightness" : "dayBrightness";
        const current = this[key] || (isNight ? 90 : 100);
    ...
    setupKeybindings() {
        ...
        if (this.keyTempUp) {
            Main.keybindingManager.addHotKey("redshift-temp-up", this.keyTempUp, () => {
                this.adjustActiveTemperature(TEMP_STEP);
            });
        }
        if (this.keyTempDown) {
        ...
        if (this.keyBrightnessUp) {
            Main.keybindingManager.addHotKey("redshift-brightness-up", this.keyBrightnessUp, () => {
                this.adjustActiveBrightness(BRIGHTNESS_STEP);
            });
        }
    ...
    update() {
        const enabled = this.enabled;
        const isNight = this.isNightTime();
        if (this._lastPeriodIsNight !== undefined && this._lastPeriodIsNight !== isNight && enabled) {
            this._lastAppliedTemp = null;
            this._lastAppliedBrightness = null;
            this.applyRedshiftNow();
        }
        this._lastPeriodIsNight = isNight;
    ...
    _clearApplyRedshiftTimeout() {
        if (this._applyRedshiftTimeoutId) {
            imports.mainloop.source_remove(this._applyRedshiftTimeoutId);
            this._applyRedshiftTimeoutId = 0;
        }
    }

    _scheduleApplyRedshift(immediate = false) {
        if (immediate) {
            this._clearApplyRedshiftTimeout();
            this._runApplyRedshift();
            return;
        }
        if (this._applyRedshiftTimeoutId)
            return;
        this._applyRedshiftTimeoutId = imports.mainloop.timeout_add(APPLY_DEBOUNCE_MS, () => {
            this._applyRedshiftTimeoutId = 0;
            this._runApplyRedshift();
            return false;
        });
    ...
    applyRedshift() {
        this._scheduleApplyRedshift(false);
    }

    applyRedshiftNow() {
        this._scheduleApplyRedshift(true);
    }
    ...
    _scheduleReapplyAfterDisplayChange() {
        ...
        const now = Date.now();
        if (now - this._lastMonitorReapplyTime < MONITOR_REAPPLY_COOLDOWN_MS)
            return;
        ...
        this._displayChangeTimeoutId = imports.mainloop.timeout_add(500, () => {
            ...
            const reappliedAt = Date.now();
            if (reappliedAt - this._lastMonitorReapplyTime < MONITOR_REAPPLY_COOLDOWN_MS)
                return false;

            this._lastMonitorReapplyTime = reappliedAt;
            this._lastAppliedTemp = null;
            this._lastAppliedBrightness = null;
    ...
    _runApplyRedshift() {
        ...
        if (this._lastAppliedTemp === temp &&
            this._lastAppliedBrightness === brightnessPercent &&
            this.isRedshiftActive) {
            this.update();
            return;
        }

        this._lastAppliedTemp = temp;
        this._lastAppliedBrightness = brightnessPercent;
        ...
        try {
            Util.spawnCommandLineAsync(`redshift -P -O ${temp} -b ${brightness}`);
        } catch (e) {
    ...
    resetRedshift() {
        ...
        this._lastAppliedBrightness = null;
        try {
            Util.spawnCommandLineAsync("redshift -x");
            this.isRedshiftActive = false;
        } catch (e) {
